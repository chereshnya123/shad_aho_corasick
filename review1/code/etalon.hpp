...
void BreadthFirstSearch(Vertex origin_vertex, const Graph& graph,
    ...
    while (!queue.empty()) {
        ...
        for (const auto& edge : OutgoingEdges(graph, source)) {
            visitor.ExamineEdge(edge);
            target = GetTarget(graph, edge);
...
AutomatonNode* GetTrieTransition(AutomatonNode* node, char character) {
    auto item = node->trie_transitions.find(character);
    if (item == node->trie_transitions.end()) {
        return nullptr;
    }
    return &item->second;
}
...
AutomatonNode* GetAutomatonTransition(AutomatonNode* node,
                                      ...
                                      char character) {
    auto cache_lookup = node->automaton_transitions_cache.find(character);
    if (cache_lookup != node->automaton_transitions_cache.end()) {
        return cache_lookup->second;
    }
    AutomatonNode* suffix = node;
    AutomatonNode* result = GetTrieTransition(suffix, character);
    while (result == nullptr) {
        ...
        result = GetTrieTransition(suffix, character);
    }
    node->automaton_transitions_cache[character] = result;
    return result;
}
...
std::vector<typename AutomatonGraph::Edge> OutgoingEdges(
    ...
    for (auto& item : vertex->trie_transitions) {
        edges.emplace_back(vertex, &item.second, item.first);
    }
    return edges;
}
 ...
 * автомату должны быть доступны пользователю).
 * Во-вторых, так как мы используем кэширование при переходе
 * по символу в автомате, то условная функция getNextNode не
 * может быть константной (она заполняет кэш переходов).
 * Это значит, что мы лишены возможности добавить функцию
 * "перехода между вершинами" в константный интерфейс
 * (то есть, предоставить ее пользователю константного
 * указателя на AutomatonNode).
...
public:
    ...
    void GenerateMatches(Callback on_match) const {
        for (auto id : node_->terminated_string_ids) {
            on_match(id);
        }
        auto* terminal_node = node_->terminal_link;
        while (terminal_node != nullptr) {
            for (auto id : terminal_node->terminated_string_ids) {
                on_match(id);
            }
            terminal_node = terminal_node->terminal_link;
        }
    }
...
private:
    ...
    static void AddString(AutomatonNode* root, size_t string_id,
        ...
        AutomatonNode* state = root;
        AutomatonNode* next_state;
        for (char ch : string) {
            next_state = GetTrieTransition(state, ch);
            if (next_state != nullptr) {
                state = next_state;
            } else {
                state->trie_transitions[ch];  // create default AutomatonNode
                state = &state->trie_transitions[ch];
            }
        }
        state->terminated_string_ids.push_back(string_id);
    }

    static void BuildSuffixLinks(Automaton* automaton) {
        AutomatonNode* root = &automaton->root_;
        NInternal::AutomatonGraph graph = NInternal::AutomatonGraph();
        NInternal::SuffixLinkCalculator visitor(&automaton->root_);
    ...
    static void BuildTerminalLinks(Automaton* automaton) {
        AutomatonNode* root = &automaton->root_;
        NInternal::AutomatonGraph graph = NInternal::AutomatonGraph();
        NInternal::TerminalLinkCalculator visitor(&automaton->root_);
        NTraverses::BreadthFirstSearch(root, graph, visitor);
    }
...
public:
    WildcardMatcher(NAhoCorasick::NodeReference state, size_t number_of_words,
                    size_t pattern_length,
                    std::unique_ptr<NAhoCorasick::Automaton> automaton,
                    std::map<size_t, WordInfo> id_to_word_info)
        : state_{state},
          number_of_words_{number_of_words},
          pattern_length_{pattern_length},
          aho_corasick_automaton_{std::move(automaton)},
          id_to_word_info_{id_to_word_info} {}
    ...
    WildcardMatcher static BuildFor(const std::string& pattern, char wildcard) {
        ...
        for (size_t i = 0; i < segments.size(); ++i) {
            if (!segments[i].empty()) {
                id_to_word_info[number_of_words] =
                    WordInfo{position, segments[i].length()};
                builder.Add(segments[i], number_of_words++);
            }
        ...
        NAhoCorasick::NodeReference state = automaton->Root();
        return {state, number_of_words, pattern.length(), std::move(automaton),
                id_to_word_info};
    }
    ...
    void Scan(char character, Callback on_match) {
        ...
        state_.GenerateMatches(
            [&](size_t id) { this->UpdateWordOccurrencesCounters(id); });
...
private:
    void UpdateWordOccurrencesCounters(size_t id) {
        WordInfo word_info = id_to_word_info_[id];
        if (words_occurrences_by_position_.size() >=
            word_info.position + word_info.length) {
            size_t pos = words_occurrences_by_position_.size() -
                         word_info.position - word_info.length;
            ++words_occurrences_by_position_[pos];
        }
    }
    ...
    std::map<size_t, WordInfo> id_to_word_info_;
};
...
std::vector<size_t> FindFuzzyMatches(const std::string& pattern_with_wildcards,
    ...
    for (size_t i = 0; i < text.size(); ++i) {
        matcher.Scan(text[i], [&occurrences, i,
                               length = pattern_with_wildcards.size()]() {
            occurrences.push_back(i + 1 - length);
        });
    }
    return occurrences;
}
...
void Print(const std::vector<size_t>& sequence) {
    ...
    for (size_t ind = 0; ind < (sequence.size() - 1); ++ind) {
